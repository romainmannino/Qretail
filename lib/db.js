...
export async function getTag(token){
  const r=await fetch(URL+"/rest/v1/qr_tags?token=eq."+encodeURIComponent(token)+"&select=*,products(*,brands(*),catalogs(*))",{headers,cache:"no-store"});
...
export async function getProducts(search=""){
  ...
  let url=URL+"/rest/v1/products?select=*,brands(*),catalogs(*)&order=name.asc&limit=200";
  if(term){
...
export async function getModelVariants(model,catalogId=""){
  const m=String(model||"").trim();
  if(!m)return [];
  let url=URL+"/rest/v1/products?select=*,brands(*),catalogs(*)&model=eq."+encodeURIComponent(m);
  if(catalogId)url+="&catalog_id=eq."+encodeURIComponent(catalogId);
  url+="&order=name.asc&limit=500";
  const r=await fetch(url,{headers,cache:"no-store"});
  if(!r.ok)throw new Error("getModelVariants "+r.status+" "+await r.text());
  const d=await r.json();
  return Array.isArray(d)?d:[];
}
export async function getModelColorVariants(model,catalogId=""){
  const rows=await getModelVariants(model,catalogId);
  const seen=new Set();
