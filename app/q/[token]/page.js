...
export default function Product(){
 ...
 useEffect(()=>{
   ...
   (async()=>{try{
     ...
     if(!alive)return;
     setTag(data);
     if(data?.product_id){track(data).catch(()=>{});const m=data?.products?.model;if(m)getModelColorVariants(m).then(v=>{if(alive)setColorVariants(v)}).catch(()=>{})}
 ...
 return <main className="consumer"><header className="productTop"><Logo/><span className="screenOnly">En magasin</span></header><section className="productHero"><div className="brand">{b.name||"Marque"}</div><div className="printBrand"><Logo/></div><h1>{p.model||p.name}</h1>{price>0&&<div className="productPrice">{euro(price)} <small>Prix public conseillé</small></div>}<div className="productVisuals">{p.image_url&&<img className="productImage" src={p.image_url} alt={p.model||p.name}/>}<div className="alternateColors">{colorVariants.filter(v=>v.image_url!==p.image_url).slice(0,3).map(v=>{const vs=v.specs?.source_row||{},vc=vs["Summarised Colour (EN)"]||vs["Orbea Colour (EN)"]||v.color_code;return <div className="alternateColor" key={v.id}><img src={v.image_url} alt={vc}/><small>{vc}</small></div>})}</div></div><div className="meta">{size&&<span>Taille {size}</span>}{color&&<span>{color}</span>}{p.specs?.product_code&&<span className="printOnly">Réf. {p.specs.product_code}</span>}</div>{primaryComponents.length>0&&<div className="components"><h2>Équipement</h2>{primaryComponents.map(([k,v])=><div className="componentRow" key={k}><span>{k}</span><b>{v}</b></div>)}<div className="printSecondary">{secondaryComponents.map(([k,v])=><div className="componentRow" key={k}><span>{k}</span><b>{v}</b></div>)}{showSpecs&&secondaryComponents.map(([k,v])=><div className="componentRow screenOnly" key={k}><span>{k}</span><b>{v}</b></div>)}{secondaryComponents.length>0&&<button className="specToggle" onClick={()=>setShowSpecs(!showSpecs)}>{showSpecs?"Réduire":"Voir tous les composants"}</button>}</div>}<div className="sheetActions"><button className="primary full" onClick={printSheet}>Télécharger la fiche PDF</button><button className="save full" onClick={()=>setForm(!form)}>Recevoir cette fiche par e-mail</button></div>{p.product_url&&<a className="textBtn productLink" href={p.product_url} target="_blank" rel="noreferrer">Voir la fiche officielle {b.name||""}</a>}</section>
