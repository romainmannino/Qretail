...
export default function Product(){
 ...
 return <main className="consumer"><header className="productTop"><Logo/><span>{tag.retailers?.name||"En magasin"}</span></header><section className="productHero"><div className="brand">{b.name||"Marque"}</div><h1>{p.name}</h1><div className="meta">{p.color_code&&<span>Coloris {p.color_code}</span>}<span>Fiche officielle</span></div>{p.description&&<p>{p.description}</p>}{p.product_url&&<a className="primary full" href={p.product_url} target="_blank" rel="noreferrer">Voir la fiche {b.name||""}</a>}<button className="save full" onClick={()=>setSaved(!saved)}>{saved?"✓ Produit conservé":"Conserver ce produit"}</button><button className="textBtn" onClick={()=>setForm(!form)}>Recevoir la fiche et être recontacté</button></section>
