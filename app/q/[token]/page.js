...
export default function Product(){
 ...
 return <main className="consumer"><header className="productTop"><Logo/><span>{tag.retailers?.name||"En magasin"}</span></header><section className="productHero"><div className="brand">{b.name||"Marque"}</div><h1>{p.model||p.name}</h1>{price>0&&<div className="productPrice">{euro(price)} <small>Prix public conseillé</small></div>}{p.image_url&&<img className="productImage" src={p.image_url} alt={p.model||p.name}/>}<div className="meta">{size&&<span>Taille {size}</span>}{color&&<span>{color}</span>}</div>{components.length>0&&<div className="components"><h2>Équipement</h2>{components.slice(0,5).map(([k,v])=><div className="componentRow" key={k}><span>{k}</span><b>{v}</b></div>)}{showSpecs&&components.slice(5).map(([k,v])=><div className="componentRow" key={k}><span>{k}</span><b>{v}</b></div>)}{components.length>5&&<button className="specToggle" onClick={()=>setShowSpecs(!showSpecs)}>{showSpecs?"Réduire":"Voir tous les composants"}</button>}</div>}<div className="sheetActions"><button className="primary full" onClick={printSheet}>Télécharger la fiche PDF</button><button className="save full" onClick={()=>setForm(!form)}>Recevoir cette fiche par e-mail</button></div>{p.product_url&&<a className="textBtn productLink" href={p.product_url} target="_blank" rel="noreferrer">Voir la fiche officielle {b.name||""}</a>}</section>
