...
export default function Configure(){
 ...
 return <main className="retailerApp">
  ...
  <section className="retailerBody">
   ...
   <p>Le QR reste le même. Vous pouvez modifier sa destination à tout moment.</p>
   <div className="destinationChoices">
    <button type="button" className={destination==="product"?"active":""} onClick={()=>{setDestination("product");setError("")}}><b>Fiche produit</b><span>Créer la fiche à partir du catalogue / ePOS.</span></button>
    <button type="button" className={destination==="url"?"active":""} onClick={()=>{setDestination("url");setError("")}}><b>Redirection vers un site</b><span>Ouvrir directement une URL externe.</span></button>
   </div>
   {tag?.destination_type==="url"&&tag?.destination_url?<div className="current">Destination actuelle : URL externe</div>:tag?.product_id?<div className="current">Destination actuelle : fiche produit.</div>:null}
   {destination==="url"&&<div className="urlDestination">
    <label>Adresse du site</label>
    <input value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://www.monsite.fr/fiche-produit"/>
    <p className="urlHint">Le QR physique ne change pas : seule cette destination sera mise à jour.</p>
    {error?<p className="scanError">{error}</p>:null}
    <button type="button" className="primary full stickySave" disabled={!url.trim()} onClick={saveUrl}>Associer cette URL à l’affiche</button>
   </div>}
   {destination==="product"&&<>
    <label>Rechercher dans le catalogue</label>
    <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Ex. Orca M30"/>
    <div className="productList">
     {!modelChoice&&list.map(p=><button type="button" key={p.id} className="productChoice" onClick={async()=>{const m=p.model||p.name;setModelChoice(m);setCatalogChoice(p.catalog_id||"");setSize("");setColor("");try{const all=await getModelVariants(m,p.catalog_id||"");setProducts(all)}catch(e){console.error("VARIANT_LOAD",e)}}}><span><b>{p?.model||p?.name||"Produit"}</b><small>{p?.brands?.name||"Orbea"} · MY{p?.catalogs?.year||"—"} · {p?.specs?.family||"Catalogue"}</small></span><i>›</i></button>)}
    </div>
    {modelChoice&&<div className="variantPicker">
     <button type="button" className="backModel" onClick={()=>{setModelChoice(null);setCatalogChoice("");setSelected(null);setSize("");setColor("")}}>← Changer de modèle</button>
     <h3>{modelChoice} <small>· MY{variants[0]?.catalogs?.year||"—"}</small></h3>
     <label>Taille <small>facultatif</small></label>
     <div className="optionChips"><button type="button" className={!size?"active":""} onClick={()=>setSize("")}>Toutes</button>{sizes.map(v=><button type="button" key={v} className={size===v?"active":""} onClick={()=>setSize(v)}>{v}</button>)}</div>
     <label>Coloris <small>facultatif</small></label>
     <div className="optionChips"><button type="button" className={!color?"active":""} onClick={()=>setColor("")}>Tous</button>{colors.map(v=><button type="button" key={v.code} className={color===v.code?"active":""} onClick={()=>setColor(v.code)}>{v.label}</button>)}</div>
    </div>}
    {q.trim().length>=2&&!modelChoice&&!list.length&&!error?<p>Aucun modèle trouvé.</p>:null}
    {error?<p className="scanError">{error}</p>:null}
    {selected?<button type="button" className="primary full stickySave" onClick={save}>Associer {selected?.name||"ce produit"} à cette affiche</button>:null}
   </>}
  </section>
 </main>;
}
